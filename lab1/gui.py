...
class Paint:
    ...
    def _setup_tools(self):
        ...
        self.mode_combobox = ttk.Combobox(self.tool_frame, values=self.mode, state="readonly")
        self.mode_combobox.current(0)
        self.mode_combobox.pack(side=tk.TOP, padx=5, pady=5)
        self.mode_combobox.bind("<<ComboboxSelected>>", lambda event: self._select_mode(self.mode_combobox.get()))
        ...
        self.color_combobox.bind("<<ComboboxSelected>>", lambda event: self._select_color(self.color_combobox.get()))
        ...
        self.debug_button = ttk.Button(self.tool_frame, text="Debug", command=self.debug_line)
    ...
    def click_handler(self, event) -> None:
        ...
        try:
            self.points, self.additional, change_flag = function(self.draw[0], self.draw[1])
        ...
        if self.selected_mode != 'Wu':
            for i in self.points:
                self.canvas.create_line(i[0], i[1], i[0] + 1, i[1] + 1, fill=self.selected_color, width=2)
        else:
            s1 = 1 if self.points[-1][0] > self.points[0][0] else -1
            s2 = 1 if self.points[-1][1] > self.points[0][1] else -1

            k = (self.points[-1][1] - self.points[0][1]) / (self.points[-1][0] - self.points[0][0])
            b = self.points[-1][1] - self.points[-1][0] * k
            for i in range(len(self.points)):
                if change_flag:
                    self.additional[i] = (
                        self.additional[i][0] - 10 * s1, self.additional[i][1],
                        abs(self.points[i][0] * k + b - self.points[i][1]))
                else:
                    self.additional[i] = (
                        self.additional[i][0], self.additional[i][1] - 10 * s2,
                        abs(self.points[i][0] * k + b - self.points[i][1]))
            for i in range(len(self.points)):
                color_1 = "#%02x%02x%02x" % (
                    abs(int(255 * self.additional[i][2])), abs(int(255 * self.additional[i][2])),
                    abs(int(255 * self.additional[i][2])))

                color_2 = "#%02x%02x%02x" % (
                    abs(int(255 * (1 - self.additional[i][2]))), abs(int(255 * (1 - self.additional[i][2]))),
                    abs(int(255 * (1 - self.additional[i][2]))))

                self.canvas.create_rectangle(self.points[i][0], self.points[i][1],
                                             self.points[i][0] + 1, self.points[i][1] + 1, fill=color_1)
                self.canvas.create_rectangle(self.additional[i][0], self.additional[i][1], self.additional[i][0] + 1,
                                             self.additional[i][1] + 1, fill=color_2)
        delattr(self, 'draw')
    ...
    def debug_line(self):
        ...
        self.debug_window.geometry("600x600")

        next_button = Button(self.debug_window, text="Next")
        next_button.grid()

        debug_canvas = Canvas(self.debug_window, width=600, height=600, background="white")
        debug_canvas.grid()
        if self.selected_mode == 'Wu':
            ...
            for i in range(len(self.additional)):
                if pixels[i][0] == prev_x:
                    ...
                    self.additional[i] = (pixels[i][0] + 10 * x * sign_x, prev_add_y, self.additional[i][2])
                ...
                if pixels[i][1] == prev_y:
                    ...
                    self.additional[i] = (prev_add_x, pixels[i][1] + 10 * y * sign_y, self.additional[i][2])
        ...
        def debug_draw(*args):
            if self.selected_mode != 'Wu':
                debug_canvas.create_rectangle(self.points[0][0],
                                              self.points[0][1],
                                              self. points[0][0] + 10,
                                              self.points[0][1] + 10,
                                              fill="black")
                ...
                debug_canvas.create_rectangle(self.points[0][0], self.points[0][1], self.points[0][0] + 10,
                                              self.points[0][1] + 10, fill=color_1)
                debug_canvas.create_rectangle(self.additional[0][0], self.additional[0][1], self.additional[0][0] + 10,
                                              self.additional[0][1] + 10, fill=color_2)
                self.points.pop(0)
